...
const CreateQuiz = () => {
  ...
  if (!quiz.title || quiz.questions.length < 1) {
  ...
  const quizPayload = {
    ...
    durationSeconds: 300,
    level: quiz.difficulty.toUpperCase(), // ✅ bắt buộc uppercase
    createdBy: 1,
    createdAt: new Date().toISOString(),
    active: true,
    questions: quiz.questions.map((q, index) => ({
      id: 0,
      content: q.question || `Question ${index + 1}`,
      answers: q.options
        .filter(opt => opt.trim() !== "")
        .map(opt => ({
          id: 0,
            ...
            <div className="lg:col-span-2 space-y-6">
              ...
              {quiz.questions.length > 0 && (
                <Card className="bg-gradient-glass backdrop-blur-sm border-white/10">
                  <CardHeader>
                    <CardTitle>
                      Questions ({quiz.questions.length}/10 minimum)
                    </CardTitle>
                  </CardHeader>
                  ...
                  {sampleAlbums.map((album) => (
                    ...
                    >
                      ...
                      <Button
                        size="sm"
                        variant="outline"
                        onClick={() => createQuizFromAlbum(album)}
                      >
                        Use
                      </Button>
                    </div>
