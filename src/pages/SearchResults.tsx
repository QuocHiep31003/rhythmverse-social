...
const SearchResults = () => {
  const { playSong: playGlobalSong, setQueue } = useMusic();
  ...
  const filters = [
    { id: "all", label: "All", count: searchResults.songs.length + searchResults.artists.length + searchResults.albums.length },
  ...
  const playSong = (song: any) => {
    ...
    const formattedQueue = searchResults.songs.map((s: any) => ({
      id: s.id,
      title: s.name || s.title,
      artist: s.artists?.map((a: any) => a.name).join(", ") || "Unknown",
      album: s.album?.name || "",
      duration: s.duration || 0,
      cover: s.urlImageAlbum || s.cover || "",
      audioUrl: s.audioUrl || s.audio || "",
    }));

    setQueue(formattedQueue);
    playGlobalSong(formattedSong);
  };
          ...
          <div className="flex gap-2 overflow-x-auto scrollbar-hide">
            {filters.map((filter) => (
              <Button
                ...
                className={`rounded-full px-4 py-2 whitespace-nowrap ${activeFilter === filter.id
